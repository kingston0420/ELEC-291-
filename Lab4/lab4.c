...
{   
    ...
    while (1)
    {
		...
		capacitance = period*1.44/(1960*3);
		
		sprintf(printcap, "Sav and Kingston");
	    LCDprint(printcap, 2, 0);
		
		sprintf(printcap, "Cap.=%.3f uF    ", capacitance*1000000.0);
	    LCDprint(printcap, 1, 0);
		sprintf(printcap, "Cap.=%.3f uF    ", capacitance*1000000.0);
		waitms(100);
		
		//sprintf(printcap, "T=%.3f F=%.3f  ", period, 1.0/period);
	    //LCDprint(printcap, 2, 0);
		//sprintf(printcap, "T=%.3f F=%.3f  ", period, 1.0/period);
    }
